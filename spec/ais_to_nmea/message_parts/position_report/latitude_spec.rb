...
RSpec.describe AisToNmea::MessageParts::PositionReport::Latitude do
  ...
  it 'accepts a valid value' do
    part = described_class.new(48.8566)
    expect(part.validate!).to eq(part)
  end

  it 'rejects an invalid value' do
    expect { described_class.new(91.0).validate! }.to raise_error(AisToNmea::InvalidFieldError)
  end

  describe '#pack' do
    subject(:message_part) { described_class.new(48.8566) }

    it 'packs value into AIS bits' do
      expect(message_part.pack.length).to eq(27)
    end
  end
end
