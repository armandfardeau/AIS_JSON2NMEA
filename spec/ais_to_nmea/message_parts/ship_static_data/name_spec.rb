...
RSpec.describe AisToNmea::MessageParts::ShipStaticData::Name do
  it 'normalizes the input value' do
    expect(described_class.new(123).value).to eq('123')
  end

  it 'accepts a valid value' do
    part = described_class.new('VESSEL')
  ...
  describe '#pack' do
    subject(:message_part) { described_class.new('BOAT') }

    it 'packs value into AIS bits' do
      expect(message_part.pack.length).to eq(120)
    end
  end
end
